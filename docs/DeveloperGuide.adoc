...
image::Card_Tag_relation.png[width="650"]

==== Current Implementation
Edges are stored in an instance of `CardTag`. `CardTag` is an
association class, containing two `HashMap`s.

image::CardTag_Association_Class.png[width="650"]

As of now, no additional metadata is stored for the relations between flashcards and tags.
Examples of such metadata would include time of association between flashcard and tag.
Hence, using HashMaps of IDs to IDs suffice.

As with `UniqueCardList` and `UniqueTagList`, only one instance of `CardTag` exists in each
`AddressBook`.
...
image::addTags_SQ.svg[width="800"]
...
model edges as UUID pairs.
  ...
  - Cons: Requires the addition of a UUID field to the `Card` and
    ...
    and Object-Relational Mapping (ORM) tool were used.
