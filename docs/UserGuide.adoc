...
Format: `edit INDEX [n/NAME] [p/PHONE] [e/EMAIL] [a/ADDRESS] [t/TAG]...`
     OR `edit INDEX [n/NAME] [p/PHONE] [e/EMAIL] [a/ADDRESS] [t/TAG]...`
...
* `delete 1` +
`list` +
`undo` (reverses the `delete 1` command) OR
...
* `delete 1` +
`clear` +
`undo` (reverses the `clear` command) OR
...
`redo` (reapplies the `delete 1` command) OR
...
`redo` (reapplies the `delete 1` command) OR
`r` (reapplies the `delete 1` command) +
`redo` (reapplies the `clear` command) OR
